...
def run_mirna_simulation_suite(config: SimulationConfig) -> dict[str, Any]:
    ...
    general_result = run_general_mirna_experiment(
        mirna_sequences,
        gene_sequences,
        max_iter=config.simple_max_iter,
    ...
    summary = {
        ...
        "simple_model": summarize_params(simple_result["params"]),
        "step_length_experiment": summarize_step_results(step_result),
        "replicate_experiment": summarize_replicate_results(replicate_result),
        "general_model": summarize_general_result(general_result),
    }
...
def run_general_mirna_experiment(
    mirna_sequences: list[Seq],
    gene_sequences: list[Seq],
    *,
    max_iter: int,
    ...
    if make_plots:
        plot_histogram(
            ...
            title="General model alignment scores",
        )
        plot_histogram(
            logit_scores,
            figures_dir / "general_logit_scores_hist.png",
            title="General model logit scores",
        )
        plot_optimization_trajectories(
            params,
            max_iter=max_iter,
    ...
    return {
        "truth": truth,
        "true_substitution": true_substitution,
        "scores": scores,
        "logit_scores": logit_scores,
        "labels": labels,
        "true_loglik": true_loglik,
...
def summarize_step_results(step_result: dict[str, Any]) -> dict[str, Any]:
    summaries = []
    for step_length, params in zip(
        step_result["step_lengths"],
        step_result["results"],
    ):
        summaries.append(
            {
    ...
    return {
        "true_loglik": as_float(step_result["true_loglik"]),
        "runs": summaries,
    }


def summarize_replicate_results(replicate_result: dict[str, Any]) -> dict[str, Any]:
    final_logliks = [
        as_float(params.get("final_loglik"))
        for params in replicate_result["results"]
    ]
    final_logliks = [
        value
        for value in final_logliks
        if value is not None
    ]

    return {
        "replicate_count": len(replicate_result["results"]),
        "mean_true_loglik": float(np.mean(replicate_result["true_logliks"])),
...
def summarize_general_result(general_result: dict[str, Any]) -> dict[str, Any]:
    return {
        **summarize_params(general_result["params"]),
        "true_loglik": as_float(general_result["true_loglik"]),
        "substitution_correlation": general_result["substitution_comparison"][
            "correlation"
        ],
        "substitution_mean_absolute_error": general_result[
            "substitution_comparison"
        ]["mean_absolute_error"],
    }
...
def plot_histogram(values: np.ndarray, output_path: Path, *, title: str) -> None:
    plt.figure()
...
def plot_optimization_trajectories(
    ...
    *,
    ...
    plt.plot([max_iter // 2, max_iter], [0, 0], "k--")
    plt.title("Subgradient L2 norm trajectory")
    ...
    plt.plot([max_iter // 2, max_iter + 1], [true_loglik, true_loglik], "k--")
    plt.title("LogLikelihood trajectory")
...
def plot_step_length_loglikelihoods(
    ...
    *,
    ...
    plt.plot([0, max_iter], [true_loglik, true_loglik], "k--")
