...
def _fit_logistic_regression(logit, predictors, labels):
...
def _initial_estim_affinegap_simplesubs(alignment_list, labels):
    ...
    _fit_logistic_regression(logit, predictors, labels)
...
def _initial_estim_affinegap_fullsubs(
    alignment_list,
    labels,
    alphabet,
):
    ...
    substitution_matrix = substitution_matrices.Array(
        data=np.zeros((Asize, Asize)),
        alphabet=alphabet,
    )
    for char1 in alphabet:
        for char2 in alphabet:
            substitution_matrix[char1, char2] = logit.coef_[0][
                pair_to_id[(char1, char2)]
            ]
...
def get_initial_estimate(
    ...
    assert gap_mode in {
        "affine",
        "linear",
    }, "Only linear and affine gap modes are supported"
    assert substitution_mode in {"simple", "symmetric", "general"}

    if substitution_mode != "simple":
        assert (
            alphabet is not None
        ), "General and symmetric substitution mode require to specify the alphabet"

    if substitution_mode == "simple":
        if gap_mode == "affine":
            estimates = _initial_estim_affinegap_simplesubs(
                alignment_list,
                labels,
            )
        elif gap_mode == "linear":
            estimates = _initial_estim_lineargap_simplesubs(
                alignment_list,
                labels,
            )
    else:
        estimates = _initial_estim_affinegap_fullsubs(
            alignment_list,
            labels,
            alphabet,
        )
        ...
        if gap_mode == "linear":
            estimates["gap_score"] = (
                estimates["open_gap_score"] + estimates["extend_gap_score"]
            )
